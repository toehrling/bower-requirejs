  ...
  function writeConfig(generatedConfig) {
    ...
    requirejs.tools.useLib(function (require) {
      rjsConfig = require('transform').modifyConfig(config, function (config) {
        ...
        // cleanup config from bower-require opts
        delete generatedConfig.config;
        delete generatedConfig.exclude;
        delete generatedConfig.transitive;
        delete generatedConfig.excludeDev;
        delete generatedConfig.bowerComponentPath;

        return generatedConfig;
      });

      fs.writeFileSync(configPath, rjsConfig, {encoding: 'utf-8', flag: 'w'});
